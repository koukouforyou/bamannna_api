...
namespace Bamanna.DouDian.DouDianClient
{
    public class AccessTokenManager : IAccessTokenManager, ISingletonDependency
    {
        ...
        public async Task<AbpAuthenticateResultModel> LoginAsync()
        {
            ...
            using (var client = CreateDouDianClient())
            {
        ...
        private static IFlurlClient CreateDouDianClient()
        {
            IFlurlClient client = new FlurlClient(DouDianUrlConfig.BaseUrl);
            client.WithHeader("Accept", new MediaTypeWithQualityHeaderValue("application/json"));
            client.WithHeader("User-Agent", DouDianConsts.AbpDouDianClientUserAgent);
            return client;
        }
    }
}